...
def plot_multiple_csvs_with_logging(folder_path,
                                    ...
                                    output_log="fitted_parameters.txt", x_limits=None, y_limits=None):
    # Match figure size
    plt.figure(figsize=(6, 5))
    files = sorted(glob.glob(os.path.join(folder_path, "*.csv")))
    color_map = plt.get_cmap('tab10')  # consistent colormap
            ...
            if fit_type == 'exponential':
                ...
                if params is not None:
                    ...
                    plt.plot(fit_time, exponential(fit_time, *params),
                             label=f"{label_base} Fit", linestyle='--', linewidth=1.5, color=color)
                ...
                if params is not None:
                    A, k, c, m = params
                    eA, ek, ec, em = errors
                    plt.plot(fit_time, single_exponential_with_drift(fit_time, *params),
                             label=f"{label_base} Drift Fit", linestyle='--', linewidth=1.5, color=color)
                    t_half = np.log(2)/k if k > 0 else np.nan
                ...
                if params is not None:
                    ...
                    plt.plot(fit_time, double_exponential(fit_time, *params),
                             label=f"{label_base} Double Fit", linestyle='--', linewidth=1.5, color=color)
                ...
                plt.plot(fit_time, slope * fit_time + intercept,
                         label=f"{label_base} Linear Fit", linestyle='--', linewidth=1.5, color=color)
