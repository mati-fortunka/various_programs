...
def extract_fluorescence_and_plot(folder_path, concentration_file, smoothing_method=None, window_size=15, spline_smoothing_factor=0.5, poly_order=3, baseline_wavelength=None, show_fit=True):
        ...
        if file_name.endswith('.csv'):
            ...
            try:
                ...
                if den_concentration is not None:
                    ew_vs_concentration.append((den_concentration, ew))
    ...
    plot_data = pd.DataFrame(ew_vs_concentration, columns=['den_concentration', 'EW'])
    ...
    try:
        ...
        for _ in range(len(popt)):
            print(f"{popt[_]} ± {perr[_]}")
    ...
    plt.scatter(plot_data['den_concentration'], plot_data['EW'], label='Data', color='b', marker='o')
    if popt is not None and show_fit==True:
        fitted_y = G(x_data, *popt)
        plt.plot(x_data, fitted_y, label='Fit', color='r', linestyle='-')
    plt.title('Equivalent Width (EW) vs Denaturant Concentration')
    plt.xlabel('Denaturant Concentration (M)')
    plt.ylabel('Equivalent Width (a.u.)')
    plt.legend()
    plt.grid(True)
    output_file = folder_path + f"/EW_vs_Denaturant_{smoothing_method}_fit.png"
    plt.savefig(output_file)
    plt.show()


# Example usage
folder_path = "/home/matifortunka/Documents/JS/data_Cambridge/6_3/equilibrium/GdmCl/refolding/2nd_series/fluo/6_3 WI11 ref GdHCl 2 uM 36h/best"  # Replace with the path to your folder containing CSV files
...
extract_fluorescence_and_plot(folder_path, concentration_file, smoothing_method, window_size, spline_smoothing_factor, poly_order, baseline_value, fit)
