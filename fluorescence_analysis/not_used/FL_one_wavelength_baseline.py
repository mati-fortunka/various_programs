...
def extract_fluorescence_and_plot(folder_path, wavelength, concentration_file, smoothing_method=None, window_size=15, spline_smoothing_factor=0.5, poly_order=3, baseline_wavelength=None, show_fit=True):
            ...
            try:
                ...
                if target_intensity.size > 0:
                    ...
                    if den_concentration is not None:
                        fluorescence_vs_concentration.append((den_concentration, corrected_intensity))
    ...
    plot_data = pd.DataFrame(fluorescence_vs_concentration, columns=['den_concentration', 'Intensity'])
    ...
    plt.scatter(plot_data['den_concentration'], plot_data['Intensity'], label='Data', color='b', marker='o')
    if popt is not None and show_fit==True:
        fitted_y = G(x_data, *popt)
        plt.plot(x_data, fitted_y, label='Fit', color='r', linestyle='--')
    plt.title(f'Fluorescence Intensity at {wavelength} nm vs Denaturant Concentration')
...
extract_fluorescence_and_plot(folder_path, wavelength, concentration_file, smoothing_method, window_size, spline_smoothing_factor, poly_order, baseline_wavelength, fit)
