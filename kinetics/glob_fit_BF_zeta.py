...
def get_all_files(folder, protein):
    ...
    search = 'dzeta' if protein == 'dzeta' else protein
...
if __name__ == "__main__":
    ...
    cd_spectra_file = "/home/matifortunka/Documents/JS/data_Cambridge/8_3/Z/spectra_kinetics/60h/8_3_zeta_spectra_kin_60h00000_t.csv"
    cd_kinetics_folder = f"/home/matifortunka/Documents/JS/data_Cambridge/8_3/paper/SI_plots/CD_kinetics/new_{PROTEIN}"

    CUT_CONFIG = {
        'SF_Fast': {'min': 2.0, 'max': 9.965},
        'SF_Slow': {'min': 2.0, 'max': None},
        ...
        for i, fpath in enumerate(files):
            ...
            if t_raw is not None:
                ...
                if t_cut is not None and len(t_cut) > 10:
                    ...
                    params.add(scale_key, value=1.0, vary=False)
                    data_blocks.append({
    ...
    # PARAMS
    params.add('k_B', value=np.log(2) / 11.0, min=0)
    params.add('k_C', value=np.log(2) / 353.0, min=0)
    params.add('k_D', value=0.01, min=0)
    params.add('t_half_D', value=1650.0, min=1000, max=5000)
    params.add('k_E', value=np.log(2) / (1.4 * 3600), min=0)
    params.add('k_F', value=np.log(2) / (25 * 3600), min=0)
    ...
    # Define lists exactly as they are in the objective function
    phases = ['B', 'C', 'D', 'E', 'F']
    ...
    plt.tight_layout()
    plt.savefig(f"/home/matifortunka/Documents/JS/data_Cambridge/8_3/paper/SI_plots/global fitting/zeta_new.svg",
                format='svg', dpi=500, bbox_inches='tight', transparent=True)
    plt.show()