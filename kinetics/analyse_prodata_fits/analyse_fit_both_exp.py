...
def load_and_process_data(filename, column_to_average=[7, 9], output_plot='averaged_plot.png'):
    ...
    key_column = df.columns[0]
    value_column = df.columns[column_to_average[0]]
    value_column2 = df.columns[column_to_average[1]]
    ...
    df_filtered = df.groupby(key_column)[value_column2].apply(remove_outliers).reset_index()
    df_grouped = df_filtered.groupby(key_column)[value_column2].agg(['mean', 'std']).dropna()
    print(df_grouped)
    df_grouped['ln_mean'] = np.log(df_grouped['mean'])
    df_grouped['ln_std'] = df_grouped['std'] / df_grouped['mean']
    ...
    plt.errorbar(df_grouped.index, df_grouped['ln_mean'], yerr=df_grouped['ln_std'], fmt='o', label='Data2')
    plt.plot(df_grouped.index, linear_fit(df_grouped.index, *popt), linestyle='-',
             label=f'Fit2: y = {popt[0]:.4f}x + {popt[1]:.4f}')
    plt.xlabel("Urea Concentration [M]")
    plt.ylabel("ln(k)")
    plt.title("Ln(k) of averaged data over urea concentration")
    plt.legend()
...
out = fname.rstrip("fit.txt") + "_both"
load_and_process_data(fname, column_to_average=[7,9], output_plot=out)
