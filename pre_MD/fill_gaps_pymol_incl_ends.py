...
def find_gaps(pdb_file):
    ...
    for chain in chains:
        ...
        # Identify the range of residues
        if residue_ids:
            ...
            if first_residue > 1:
                gaps.append((1, first_residue - 1, chain))

            # Check for gaps within the chain
            ...
            # Assume an upper bound for the maximum residue number for gap detection
            max_residue_number = 2000
            if last_residue < max_residue_number:
                gaps.append((last_residue + 1, max_residue_number, chain))
