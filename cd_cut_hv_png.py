...
input_folder = "/home/matifortunka/Documents/JS/data_Cambridge/MateuszF/Yibk_CD/near/"  # Replace with your folder path
...
def process_csv(file_path, output_folder, hv_threshold):
    ...
    filtered_txt_file_path = os.path.join(output_folder, f"{base_filename}_filtered.txt")
    ...
    plt.plot(filtered_wavelength, filtered_ellipticity_avg, label='Filtered Ellipticity [mdeg]', color='blue')
    plt.xlabel('Wavelength [nm]')
    plt.ylabel('Ellipticity [mdeg]')
    plt.title(f'Filtered Circular Dichroism (HV ≤ {hv_threshold} V)')
    plt.grid(True)
    plt.legend()
    plt.tight_layout()

    # Save the plot as a PNG file
    png_file_path = os.path.join(output_folder, f"{base_filename}_filtered.png")
    plt.savefig(png_file_path)
    plt.close()

    print(f"Processed: {file_path}")
    print(f"  Raw data saved to: {raw_txt_file_path}")
    print(f"  Filtered data saved to: {filtered_txt_file_path}")
