...
def load_bka_files(folder):
    ...
    for fname in os.listdir(folder):
        ...
        # Parse concentration from filename (match "golay<conc>m")
        match = re.search(r"golay([\d\.]+)m", fname)
        if match:
...
if __name__ == "__main__":
    path = "/home/matifortunka/Documents/JS/data_Cambridge/fusions/fusion_Tm1570_cent/tm_unfold_SAV"
    analyse_bka_and_plot(path, 215,
                         smoothing_method="savitzky_golay",
                         window_size=15, spline_smoothing_factor=0.5,
