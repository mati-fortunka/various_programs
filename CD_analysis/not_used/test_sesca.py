...
# === SESCA Deconvolution ===
structure_content = {"Time_hr": [], "alpha": [], "beta": [], "other": []}
...
for label in ["alpha", "beta", "other"]:
    ...
    try:
        if fit_model == "double":
            popt, pcov = curve_fit(double_exp, t_fit, y_fit,
                                   p0=(y_fit[0], 0.001, y_fit[0]/2, 0.0001, y_fit[-1]), maxfev=5000)
            print(f"\nFit for {label}:")
